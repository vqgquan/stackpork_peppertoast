...
function colorFor(subject) { return SUBJECT_COLORS[subject] || DEFAULT_COLOR }
...
function layoutDayEvents(events) {
  ...
  for (const e of sorted) {
    if (current.length === 0 || e.startMin < currentEnd) {
      current.push(e); currentEnd = Math.max(currentEnd, e.endMin)
    } else { clusters.push(current); current = [e]; currentEnd = e.endMin }
  }
  if (current.length) clusters.push(current)
  const result = []
  for (const cluster of clusters) {
    const trackEnds = [], clusterPositioned = []
    ...
    <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1.5">
              {session.subject && <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${colors.badge}`}>{session.subject}</span>}
              <span className="text-xs text-slate-400">{session.day_of_week} · {formatTime(session.start_time)}–{formatTime(session.end_time)}</span>
            </div>
          </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Students ({session.students.length})</p>
            {session.students.length === 0 ? (
              <p className="text-sm text-slate-400">No students enrolled in this session yet.</p>
            ) : (
              <div className="space-y-1.5">
                {session.students.map(s => (
                  <Link key={s.id} to={`/students/${s.id}`} className="flex items-center justify-between border border-slate-100 rounded-lg px-3 py-2 hover:bg-slate-50 transition-colors">
                    <span className="text-sm text-blue-700 hover:underline">{s.name}</span>
                    <svg className="w-3.5 h-3.5 text-slate-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg>
                  </Link>
                ))}
              </div>
            )}
...
function noteFor(remaining) {
  if (remaining <= 0) return { text: "Sessions exhausted — contact the student/parent to renew before the next class.", tone: "bg-red-50 text-red-700" }
  if (remaining === 1) return { text: "Only 1 session left — notify the student/parent to renew soon.", tone: "bg-amber-50 text-amber-700" }
  return { text: `${remaining} sessions left — give the student/parent a heads-up about renewing.`, tone: "bg-amber-50 text-amber-700" }
}
const NOTICES_KEY = "dashboard_dismissed_notices"
function loadDismissed() {
  try { const r = localStorage.getItem(NOTICES_KEY); return r ? new Set(JSON.parse(r)) : new Set() } catch { return new Set() }
}
function saveDismissed(set) {
  try { localStorage.setItem(NOTICES_KEY, JSON.stringify([...set])) } catch {}
}
function NoticesTable({ notices, dismissedKeys, onDismiss, onShowDismissed, dismissedCount }) {
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-6 mt-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Students with {LOW_SESSIONS_THRESHOLD} or fewer sessions remaining.</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {dismissedCount > 0 && <button onClick={onShowDismissed} className="text-xs text-slate-400 hover:text-slate-600 underline">Restore {dismissedCount} dismissed</button>}
          <span className="text-xs px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-medium">{visible.length} {visible.length === 1 ? "notice" : "notices"}</span>
        </div>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm border border-slate-200 rounded-lg table-fixed">
          <colgroup><col className="w-[6%]"/><col className="w-[11%]"/><col className="w-[44%]"/><col className="w-[31%]"/><col className="w-[8%]"/></colgroup>
          <thead><tr className="bg-slate-50 text-left text-xs text-slate-500 uppercase tracking-wide border-b border-slate-200">
            <th className="px-3 py-2">STT</th><th className="px-3 py-2">Date</th><th className="px-3 py-2">Notice Content</th><th className="px-3 py-2">Additional Note</th><th className="px-3 py-2"></th>
          </tr></thead>
          <tbody>
            {visible.length === 0 ? (
              <tr><td colSpan={5} className="px-3 py-8 text-center text-slate-400 text-sm">No students are currently running low on sessions.</td></tr>
            ) : visible.map((n, i) => (
              <tr key={n.key} className="border-t border-slate-100 align-top hover:bg-slate-50/60">
                <td className="px-3 py-2 text-slate-500 text-xs">{i + 1}</td>
                <td className="px-3 py-2 text-slate-600 text-xs whitespace-nowrap">{n.date}</td>
                <td className="px-3 py-2 text-slate-700 text-xs leading-relaxed">
                  <Link to={`/students/${n.student_id}`} className="text-blue-600 hover:underline font-medium">{n.student_name}</Link>
                ...
                <td className="px-3 py-2 text-xs"><span className={`inline-block px-1.5 py-0.5 rounded-full font-medium ${n.tone}`}>{n.note}</span></td>
                <td className="px-3 py-2 text-right"><button onClick={() => onDismiss(n.key)} className="text-xs text-slate-400 hover:text-slate-600 font-medium">Dismiss</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
// ─── Reviews table ─────────────────────────────────────────────────────────────
const SELECT_CLS = "text-sm border border-slate-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-700"
function ReviewsTable({ allReviews, classes }) {
  ...
  const subjectOptions = useMemo(() => [...new Set(classes.map(c => c.subject).filter(Boolean))].sort(), [classes])
  const classOptions   = useMemo(() => classes.filter(c => !filterSubject || c.subject === filterSubject).sort((a,b) => a.name.localeCompare(b.name)), [classes, filterSubject])
  const teacherOptions = useMemo(() => {
    const rel = allReviews.filter(r => (!filterSubject || r.subject === filterSubject) && (!filterClassId || String(r.class_id) === String(filterClassId)))
    return [...new Set(rel.map(r => r.teacher_name).filter(Boolean))].sort()
  }, [allReviews, filterSubject, filterClassId])
  function handleSubjectChange(v) { setFilterSubject(v); setFilterClassId(""); setFilterTeacher(""); setPage(1) }
  function handleClassChange(v)   { setFilterClassId(v); setFilterTeacher(""); setPage(1) }
  function handleTeacherChange(v) { setFilterTeacher(v); setPage(1) }
  const filtered = useMemo(() => allReviews.filter(r =>
  ...
  function pageNumbers() {
    const pages = [], delta = 2, left = Math.max(2, safePage - delta), right = Math.min(totalPages - 1, safePage + delta)
    pages.push(1)
  ...
  const resultBadge = filtered.length === allReviews.length ? `${allReviews.length} reviews` : `${filtered.length} of ${allReviews.length} reviews`
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 mt-6">
      <div className="flex items-start justify-between mb-4">
        <div><h2 className="text-lg font-semibold text-slate-900">Session Reviews</h2><p className="text-xs text-slate-400 mt-0.5">{resultBadge}</p></div>
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
          {[20, 50].map(n => (
            <button key={n} onClick={() => { setPageSize(n); setPage(1) }} className={`text-xs px-3 py-1.5 rounded-md font-medium transition-colors ${pageSize === n ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>{n} / page</button>
          ))}
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <select value={filterSubject} onChange={e => handleSubjectChange(e.target.value)} className={SELECT_CLS}>
          <option value="">All subjects</option>{subjectOptions.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={filterClassId} onChange={e => handleClassChange(e.target.value)} className={SELECT_CLS}>
          <option value="">All classes</option>{classOptions.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <select value={filterTeacher} onChange={e => handleTeacherChange(e.target.value)} className={SELECT_CLS}>
          <option value="">All teachers</option>{teacherOptions.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        {(filterSubject || filterClassId || filterTeacher) && (
          <button onClick={() => handleSubjectChange("")} className="text-xs text-slate-400 hover:text-slate-600 underline">Clear filters</button>
        )}
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm border border-slate-200 rounded-lg table-fixed">
          <colgroup><col className="w-[9%]"/><col className="w-[10%]"/><col className="w-[12%]"/><col className="w-[9%]"/><col className="w-[10%]"/><col className="w-[22%]"/><col className="w-[22%]"/><col className="w-[6%]"/></colgroup>
          <thead><tr className="bg-slate-50 text-left text-xs text-slate-500 uppercase tracking-wide border-b border-slate-200">
            <th className="px-3 py-2">Subject</th><th className="px-3 py-2">Session</th><th className="px-3 py-2">Class</th>
            <th className="px-3 py-2">Student</th><th className="px-3 py-2">Date</th><th className="px-3 py-2">Session Content</th>
            <th className="px-3 py-2">Review</th><th className="px-3 py-2">Result</th>
          </tr></thead>
          <tbody>
            {pageSlice.length === 0 ? (
              <tr><td colSpan={8} className="px-3 py-8 text-center text-slate-400 text-sm">No reviews match the current filters.</td></tr>
            ) : pageSlice.map(r => {
              const colors = colorFor(r.subject)
              return (
                <tr key={r.id} className="border-t border-slate-100 align-top hover:bg-slate-50/60">
                  <td className="px-3 py-2">{r.subject ? <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${colors.badge}`}>{r.subject}</span> : "—"}</td>
                  <td className="px-3 py-2 text-slate-600 text-xs">{r.day_of_week ? r.day_of_week.slice(0,3) : "—"}</td>
                  <td className="px-3 py-2 text-xs truncate"><Link to={`/classes/${r.class_id}`} className="text-blue-600 hover:underline">{r.class_name}</Link></td>
                  <td className="px-3 py-2 text-slate-800 font-medium text-xs truncate">{r.student_name}</td>
                  <td className="px-3 py-2 text-slate-600 text-xs">{r.review_date}</td>
                  <td className="px-3 py-2 text-slate-600 text-xs whitespace-pre-wrap break-words">{r.session_content || <span className="text-slate-300">—</span>}</td>
                  <td className="px-3 py-2 text-slate-600 text-xs whitespace-pre-wrap break-words">{r.review_text || <span className="text-slate-300">—</span>}</td>
                  <td className="px-3 py-2">{r.session_result ? (
                    <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${r.session_result === "Pass" ? "bg-green-50 text-green-700" : r.session_result === "Fail" ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-600"}`}>{r.session_result}</span>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          <p className="text-xs text-slate-400">Showing {(safePage-1)*pageSize+1}–{Math.min(safePage*pageSize,filtered.length)} of {filtered.length}</p>
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={safePage===1} className="text-xs px-2.5 py-1.5 rounded-md border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">← Prev</button>
            {pageNumbers().map((n,i) => n === "…"
              ? <span key={`e${i}`} className="text-xs px-2 text-slate-400 select-none">…</span>
              : <button key={n} onClick={() => setPage(n)} className={`text-xs px-2.5 py-1.5 rounded-md border font-medium transition-colors ${safePage===n ? "bg-blue-600 border-blue-600 text-white" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}>{n}</button>
            )}
            <button onClick={() => setPage(p => Math.min(totalPages,p+1))} disabled={safePage===totalPages} className="text-xs px-2.5 py-1.5 rounded-md border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">Next →</button>
          </div>
        </div>
      )}
    </div>
...
function AttendanceTable({ month, year }) {
  ...
  if (loading) return <div className="bg-white border border-slate-200 rounded-xl p-6 mt-6"><p className="text-sm text-slate-400">Loading attendance…</p></div>
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-6 mt-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Click a cell to mark attended (green) or absent (red). Save when done.</p>
        </div>
        <div className="flex items-center gap-2">
          {hasPending && (
            <span className="text-xs text-amber-600 font-medium">{Object.keys(pending).length} unsaved change{Object.keys(pending).length > 1 ? "s" : ""}</span>
          )}
          <button
            onClick={saveChanges}
...
export default function Dashboard() {
  ...
  function restoreDismissedNotices() {
    setDismissedKeys(() => { const n = new Set(); saveDismissed(n); return n })
  }

  useEffect(() => {
    Promise.all([getClasses(), getTeachers(), getStudents(), getEnrollments()])
      .then(([classData, teacherData, studentData, enrollmentData]) => {
        setClasses(classData); setTeachers(teacherData); setStudents(studentData); setEnrollments(enrollmentData)
        setLoading(false)
        const classMeta = Object.fromEntries(classData.map(c => [c.id, { class_id: c.id, class_name: c.name, subject: c.subject }]))
        Promise.all(
          classData.map(c => getClassReviews(c.id).then(reviews => reviews.map(r => ({ ...r, ...classMeta[c.id] }))).catch(() => []))
        ).then(nested => {
          setAllReviews(nested.flat().sort((a, b) => (b.review_date ?? "").localeCompare(a.review_date ?? "")))
          setReviewsLoading(false)
        })
      })
      .catch(err => { setError(err.message); setLoading(false); setReviewsLoading(false) })
  }, [])

  const sessionInfo = useMemo(() => {
    const map = {}
    classes.forEach(c => c.sessions.forEach(s => { map[s.id] = { class_id: c.id, class_name: c.name, subject: c.subject, teacher_name: s.teacher_name } }))
    return map
  }, [classes])
  ...
  const visibleClasses = classes.filter(c => c.status === "Active").filter(c => !selectedSubject || c.subject === selectedSubject)
  const eventsByDay = {}
  ...
  visibleClasses.forEach(c => {
    c.sessions.forEach(s => {
      ...
      eventsByDay[s.day_of_week].push({
        id: s.id, class_id: c.id, class_name: c.name, subject: c.subject,
        day_of_week: s.day_of_week, start_time: s.start_time, end_time: s.end_time,
        startMin: toMinutes(s.start_time), endMin: toMinutes(s.end_time),
        teacher_id: s.teacher_id, teacher_name: s.teacher_name, students: s.students,
      })
    })
  })
  ...
  return (
    <div className="p-8 w-full">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-slate-400">{selectedSubject ? `Students in ${selectedSubject}` : "Total Students"}</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <button onClick={() => setSelectedSubject("")} className={pillClasses(selectedSubject === "")}>All Subjects</button>
          {Object.entries(SUBJECT_COLORS).map(([subject, c]) => (
            <button key={subject} onClick={() => setSelectedSubject(subject)} className={pillClasses(selectedSubject === subject, c)}>{subject}</button>
          ))}
        </div>
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          <div className="overflow-y-auto" style={{ maxHeight: 640 }}>
            <div className="flex sticky top-0 z-20 bg-white border-b border-slate-200">
              ...
              {DAYS.map(day => (
                <div key={day} className="flex-1 text-center py-2 border-l border-slate-100 first:border-l-0 bg-white">
                  <p className="text-sm font-semibold text-slate-600">{day.slice(0,3)}</p>
                  <p className="text-xs text-slate-400">{weekDates[day].toLocaleDateString("en-US",{month:"short",day:"numeric"})}</p>
                </div>
              ))}
            </div>
            <div className="flex">
              <div style={{ width: GUTTER_WIDTH, height: totalHeight }} className="relative flex-shrink-0">
                {Array.from({ length: TOTAL_HOURS }).map((_,h) => (
                  <div key={h} className="absolute right-1.5 text-[12px] text-slate-400" style={{ top: h === 0 ? 2 : h * HOUR_HEIGHT - 10 }}>{hourLabel(h)}</div>
                ))}
              </div>
                  ...
                  {DAYS.map(day => (
                    <div key={day} className="relative border-l border-slate-100 first:border-l-0">
                      {layoutDayEvents(eventsByDay[day]).map(e => {
                        ...
                        const widthPct = 100/e.trackCount, leftPct = e.track*widthPct
                        const colors = colorFor(e.subject)
                        return (
                          <button key={e.id} onClick={() => setSelectedSession(e)}
                          ...
                          >
                            {e.subject && <p className={`text-[10px] font-semibold ${colors.text} truncate leading-tight`}>{e.subject}</p>}
                            <p className={`text-[10px] ${colors.text} opacity-80 truncate leading-tight`}>{e.teacher_name ?? "Unassigned"}</p>
                            <p className={`text-[9px] ${colors.text} opacity-60 truncate leading-tight`}>{formatTime(e.start_time)}–{formatTime(e.end_time)}</p>
                          </button>
                        )
                      })}
                    </div>
      ...
      <div className="flex items-center gap-3 mt-8 mb-2">
        ...
        <span className="text-base font-semibold text-slate-800 min-w-[160px] text-center">{tableMonth.label}</span>
        <button onClick={tableMonth.next} className="p-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-500">
      ...
      {selectedSession && <SessionModal session={selectedSession} teachersById={teachersById} onClose={() => setSelectedSession(null)} />}
    </div>
  )
}
